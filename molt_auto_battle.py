...
def step5_wait_final(battle_id: str, voting_battle: dict | None = None) -> dict | None:
    ...
    Strategi:
    1. Baca votingEndsAt dari data battle → hitung sisa waktu
    2. Sleep sampai timer habis + buffer 20 detik
    3. Poll setiap 15 detik max 3 menit sampai winnerId muncul
    """
    ...
    wait_before_poll = 0
    if voting_battle:
        voting_ends_str = voting_battle.get("votingEndsAt", "")
        if voting_ends_str:
            try:
                ...
                wait_before_poll = max(0, (ends_at - now_utc).total_seconds()) + 20  # +20s buffer
                mins = int(wait_before_poll // 60)
                secs = int(wait_before_poll % 60)
                log.info(f"  ⏳ Voting timer: {mins}m {secs}s tersisa → menunggu sebelum poll...")
            except Exception as e:
                log.debug(f"  Parse votingEndsAt error: {e}")

    # Fallback jika tidak ada votingEndsAt
    if wait_before_poll <= 0:
        wait_before_poll = 20  # langsung poll setelah 20s buffer
        log.info("  ⏳ votingEndsAt tidak tersedia → poll segera...")

    # ── Tunggu sampai timer habis ─────────────────────────────
    if wait_before_poll > 0:
        # Sleep bertahap agar log tidak bisu terlalu lama
        slept = 0
        report_every = 60  # log progress tiap 1 menit
        next_report  = min(report_every, wait_before_poll)
        while slept < wait_before_poll:
            chunk = min(next_report - slept, wait_before_poll - slept)
            if chunk <= 0:
                chunk = min(10, wait_before_poll - slept)
            time.sleep(chunk)
            slept += chunk
            remaining = wait_before_poll - slept
            if remaining > 5:
                log.info(f"  ⏳ Voting berlangsung... {int(remaining)}s lagi")
                next_report = slept + report_every

    # ── Poll hasil setelah timer ──────────────────────────────
    log.info("  🔍 Voting selesai → ambil hasil final...")
    max_poll = 180  # max 3 menit polling setelah timer
    elapsed  = 0
