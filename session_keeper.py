...
# Supabase publishable key MoltArena (sb_publishable_ = aman, bukan secret)
# Digunakan untuk Supabase token refresh endpoint
...
class SessionKeeper:
    ...
    def _discover_anon_key(self):
        """Ambil Supabase anon key — pakai fallback hardcoded, lalu coba discover dari halaman."""
        if self._anon_key:
            return

        # Pakai fallback hardcoded langsung (lebih cepat, tidak perlu fetch HTML)
        self._anon_key = SUPABASE_ANON_KEY_FALLBACK
        log.info("  ✅ Supabase anon key siap!")

        # Coba discover dari halaman untuk update jika key berubah
        try:
            ...
            if m and m.group(0) != self._anon_key:
                ...
                log.debug(f"  🔄 Anon key diperbarui dari halaman")
                return
            # Format lama: JWT eyJhbGci...
            for m in re.finditer(r'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9\.[A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+', r.text):
                try:
                    payload = json.loads(base64.b64decode(m.group(0).split(".")[1] + "==").decode())
                    if payload.get("role") == "anon" and payload.get("iss") == "supabase":
                        self._anon_key = m.group(0)
                        log.debug(f"  🔄 Anon key (format lama) diperbarui dari halaman")
                        return
                except Exception:
                    continue
        except Exception as e:
